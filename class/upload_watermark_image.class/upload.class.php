<?php
    ...
    class upload {
        ...
        //构造函数
        //$types : 允许上传的文件类型 , $maxsize : 允许大小 , $field : 上传input控件名称 , $time : 自定义上传时间
        ...
        //创建缩略图,以相同的扩展名生成缩略图
        //$image_file : 来源图像路径 , $thumb_file : 缩略图路径
        function create_thumb ($image_file,$thumb_file) {
            $t_width = $this->thumb_width;
            $t_height = $this->thumb_height;
            if (!file_exists($image_file)) return false;
            $image_info = getImageSize($image_file);
            //如果来源图像小于或等于缩略图则拷贝源图像作为缩略图
            if ($image_info[0] <= $t_width && $image_info[1] <= $t_height) {
                if (!copy($image_file,$thumb_file)) {
                    return false;
                }
                return true;
            }
            //按比例计算缩略图大小
            if ($image_info[0] - $t_width > $image_info[1] - $t_height) {
                $t_height = ($t_width / $image_info[0]) * $image_info[1];
            } else {
                $t_width = ($t_height / $image_info[1]) * $image_info[0];
            }
            //取得文件扩展名
            $fileext = $this->fileext($image_file);
            switch ($fileext) {
                case 'jpg' :
                    $image_img = ImageCreateFromJPEG($image_file); break;
                case 'png' :
                    $image_img = ImageCreateFromPNG($image_file); break;
                case 'gif' :
                    $image_img = ImageCreateFromGIF($image_file); break;
            }
            //创建一个真彩色的缩略图像
            $thumb_img = @ImageCreateTrueColor($t_width,$t_height);
            //ImageCopyResampled函数拷贝的图像平滑度较好，优先考虑
            if (function_exists('imagecopyresampled')) {
                @ImageCopyResampled($thumb_img,$image_img,0,0,0,0,$t_width,$t_height,$image_info[0],$image_info[1]);
            } else {
                @ImageCopyResized($thumb_img,$image_img,0,0,0,0,$t_width,$t_height,$image_info[0],$image_info[1]);
            }
            //生成缩略图
            switch ($fileext) {
                case 'jpg' :
